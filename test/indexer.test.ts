...
`)
    ...
    expect(index.findPropUsages(childUri, 'beforeConfirm').map((usage) => usage.file.uri)).toEqual([parentUri, childUri, childUri, childUri])
    expect(index.findPropUsagesFromSource(typeUri, fs.readFileSync(typeUri, 'utf8').indexOf('beforeConfirm')).map((usage) => usage.file.uri)).toEqual([parentUri, childUri, childUri, childUri])
