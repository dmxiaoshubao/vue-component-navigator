...
| 模板 prop 悬浮 | 支持 | 展示 prop 定义片段、JSDoc 摘要和定义链接。 |
...
| `@/` 别名 import | 支持 | 优先从最近的 `tsconfig.json` 或 `jsconfig.json` 读取 `compilerOptions.baseUrl` 和 `compilerOptions.paths`，并保留常见 `@/src` fallback。 |
